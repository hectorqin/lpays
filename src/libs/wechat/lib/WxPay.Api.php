...
require_once dirname(__FILE__)."/WxPay.ConfigObj.php";
...
class WxPayApi
{
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function downloadBill($inputObj, $timeOut = 6,$gzip=false)
	{
		...
		$inputObj->SetMch_id(self::$config->MCHID);//商户号
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
		if ($gzip) $inputObj->SetTar_type('GZIP');//随机字符串
		...
		if(substr($response, 0 , 5) == "<xml>"){
			if(strpos($response, 'error_code><![CDATA[20002]]>')!==false) return '';
			$response=substr($response,strpos($response, 'return_msg')+20);
			$response=substr($response,0,strpos($response, 'error_code')-18);
			throw new WxPayException($response);
		}
		if ($gzip)$response = gzdecode($response);
		return $response;
	}
	...
	public static function transfers($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsPartner_trade_noSet()){
			throw new WxPayException("转账接口中,缺少必要参数partner_trade_no！");
		}else if(!$inputObj->IsOpenidSet()){
			throw new WxPayException("转账接口中,缺少必要参数 openid！");
		}else if(!$inputObj->IsRe_user_nameSet()){
			throw new WxPayException("转账接口中,缺少必要参数 re_user_name！");
		}else if(!$inputObj->IsAmountSet()){
			throw new WxPayException("转账接口中,缺少必要参数 amount！");
		}else if(!$inputObj->IsCheck_nameSet()){
			$inputObj->SetCheck_name("OPTION_CHECK");
		}else if(!$inputObj->IsDescSet()){
			$inputObj->SetDesc('transfers money');
		}
		if (!$inputObj->IsSpbill_create_ipSet()) $inputObj->SetSpbill_create_ip(@$_SERVER['REMOTE_ADDR']);//终端ip
		$inputObj->SetAppid(self::$config->APPID);//公众账号ID
		$inputObj->SetMch_id(self::$config->MCHID);//商户号
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
	 ...
	 * 下载对账单，WxPayDownloadBill中bill_date为必填参数
	 * appid、mchid、spbill_create_ip、nonce_str不需要填入
	 * @param WxPayTransfersQuery $inputObj
	 * @param int $timeOut
	 * @throws WxPayException
	 * @return 成功时返回，其他抛异常
	 */
	public static function transfersQuery($inputObj, $timeOut = 6)
	{
		$url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers";
	...
	{
		//如果不需要上报数据
		if(self::$config->REPORT_LEVENL == 0){
			return;
		} 
		//如果仅失败上报
		if(self::$config->REPORT_LEVENL == 1 &&
	...
	{		
		...
		if(self::$config->SSLCERT_CA) curl_setopt($ch,CURLOPT_CAINFO ,self::$config->SSLCERT_CA);
