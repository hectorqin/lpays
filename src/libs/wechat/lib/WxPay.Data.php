...
class WxPayDataBase
{
	...
	**/
	...
	public function MakeSign()
	{
		...
		//签名步骤二：在string后加入KEY
		$string = $string . "&key=".WxPayApi::$config->KEY;
		//签名步骤三：MD5加密
		$string = md5($string);
		//签名步骤四：所有字符转为大写
...
class WxPayResults extends WxPayDataBase
{
	...
	public function CheckSign()
	{
		//fix异常
		if(!$this->IsSignSet()){
			throw new WxPayException("签名错误！",888);
		}
	...
	{	
		...
		if(isset($obj->values['result_code'])&&$obj->values['result_code'] != 'SUCCESS'){
			return $obj->GetValues();
		}
		$obj->CheckSign();
        return $obj->GetValues();
...
class WxPayDownloadBill extends WxPayDataBase
{
	...
	**/
	 ...
	 * @return true 或 false
	 **/
	public function IsTar_typeSet()
	{
		return array_key_exists('tar_type', $this->values);
	}
...
class WxPayTransfersQuery extends WxPayDataBase
{
	public function SetAppid($value)
	{
		$this->values['appid'] = $value;
