...
include dirname(__FILE__).'/TDESUtil.php';
...
class XMLUtil{
	...
	public static function encryptReqXml($param,$key='../config/seller_rsa_private_key.pem'){
		...
		$sign = RSAUtils::encryptByPrivateKey($sha256SourceSignString,$key);
	...
	public static function decryptResXml($resultData,&$resData,$key='../config/wy_rsa_public_key.pem'){
...
// 		$reqBodyStr = XMLUtil::xmlToString($bodyDom);
		...
		$decryptStr = RSAUtils::decryptByPublicKey($inputSign,$key);
