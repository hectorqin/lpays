...
include_once dirname(__FILE__).'/cert_util.php';
...
class AcpService {
	 ...
	 * @return 是否成功
	 */
	static function validate($params,$cert_dir=SDK_VERIFY_CERT_DIR) {
		$logger = LogUtil::getLogger();
		// 公钥
...
//	echo date('Y-m-d',time());
	 ...
	 * @return 是否成功
	 */
	static function validateAppResponse($jsonData,$cert_dir=SDK_VERIFY_CERT_DIR) {

		$data = json_decode($jsonData);
		$sign = $data->sign;
		$data = $data->data;
		$dataMap = parseQString($data);
		$public_key = CertUtil::getVerifyCertByCertId( $dataMap ['cert_id'],$cert_dir );
...
eot;
	 ...
	 *        	$customerInfo
	 */
	static function getCustomerInfoWithEncrypt($customerInfo,$cert_path=SDK_ENCRYPT_CERT_PATH) {
	  if($customerInfo == null || count($customerInfo) == 0 )
	  	return "";
		$encryptedInfo = array();
		...
		if( count ($encryptedInfo) > 0 ){
			...
			$encryptedInfo = AcpService::encryptData ( $encryptedInfo, $cert_path );
	 ...
	 */
	static function parseCustomerInfo($customerInfostr, $cert_path=SDK_SIGN_CERT_PATH, $cert_pwd=SDK_SIGN_CERT_PWD) {
		...
		if(array_key_exists("encryptedInfo", $customerInfo)) {
			...
			$encryptedInfoStr = AcpService::decryptData($encryptedInfoStr, $cert_path, $cert_pwd);
