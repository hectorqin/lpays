 ...
 */
function sign(&$params,$cert_path = SDK_SIGN_CERT_PATH) {
	$log=upacp_log();
	$log->LogInfo ( '=====签名报文开始======' );
 ...
 */
function verify($params,$cert_dir = SDK_VERIFY_CERT_DIR) {
	$log=upacp_log();
	// 公钥
	$public_key = getPulbicKeyByCertId ( $params ['certId'],$cert_dir );	
//	echo $public_key.'<br/>';
	// 签名串
 ...
 * @return string NULL
 */
function getPulbicKeyByCertId($certId,$cert_dir = SDK_VERIFY_CERT_DIR) {
	$log=upacp_log();
 ...
 * @return unknown
 */
function getCertId($cert_path,$pwd=SDK_SIGN_CERT_PWD) {
 ...
 */
function encryptPin($pan, $pwd,$cert_path = SDK_ENCRYPT_CERT_PATH) {
	$public_key = getPublicKey ( $cert_path );
 ...
 * @return unknown
 */
function encryptCvn2($cvn2,$cert_path = SDK_ENCRYPT_CERT_PATH) {
	$public_key = getPublicKey ( $cert_path );
 ...
 * @return unknown
 */
function encryptDate($certDate,$cert_path = SDK_ENCRYPT_CERT_PATH) {
	$public_key = getPublicKey ( $cert_path );
 ...
 * @return unknown
 */
function encryptDateType($certDataType,$cert_path = SDK_ENCRYPT_CERT_PATH) {
	$public_key = getPublicKey ( $cert_path );
