...
class AopClient {
	...
	public $alipayPublicKeyFilePath = null;
	...
	*/
	...
	function verify($data, $sign, $rsaPublicKeyFilePath, $signType = 'RSA') {
		...
		if($this->checkEmpty($this->alipayPublicKeyFilePath)){
	...
	function parserJSONSign($responseJSon) {
		if (!isset($responseJSon->sign)) return null;
	...
	public function checkResponseSign($request, $signData, $resp, $respObject) {
		...
		if (!$this->checkEmpty($this->alipayPublicKeyFilePath) || !$this->checkEmpty($this->alipayrsaPublicKey)) {
			...
			if (!$this->checkEmpty($responseSubCode) || ($this->checkEmpty($responseSubCode) && !$this->checkEmpty($signData->sign))) {
				...
				$checkResult = $this->verify($signData->signSourceData, $signData->sign, $this->alipayPublicKeyFilePath, $this->signType);
